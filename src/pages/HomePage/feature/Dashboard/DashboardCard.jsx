...
const RankingRow = ({ item, index, maxCount }) => {
  ...
  if (item.rank === 1) rankColor = '#ffd700'; // ทอง
  else if (item.rank === 2) rankColor = '#c0c0c0'; // เงิน
  ...
  return (
    ...
    >
      ...
      {/* กราฟแท่ง (Bar) */}
      <Box sx={{ flex: 1, mx: 2, height: '14px', backgroundColor: 'rgba(0,0,0,0.5)', border: `1px solid ${THEME.border}`, position: 'relative' }}>
        <motion.div
          initial={{ width: 0 }}
          ...
          style={{
            height: '100%',
            backgroundColor: item.rank === 1 ? '#ffd700' : item.rank === 2 ? '#c0c0c0' : item.rank === 3 ? '#cd7f32' : THEME.accent,
...
const DashboardCard = () => {
  ...
  }, [dictionaryData, searchQuery, filterType, topLimit]);
  ...
  return (
    <Box sx={{ height: "100vh", position: "relative" }}>
      ...
      >
        ...
        >
          {/* อัปเดตคำว่า TOP WORDS USED เรียบร้อยแล้วครับ */}
          <Typography
            sx={{
              fontFamily: "'Press Start 2P'",
              color: THEME.accent,
        ...
        >
          ...
          <Stack direction="row" spacing={2} sx={{ width: { xs: '100%', sm: 'auto' } }}>
            ...
            <Select
              ...
              sx={{ ...controlStyle, minWidth: '120px', flex: { xs: 1, sm: 'none' } }}
              MenuProps={{ PaperProps: { sx: { backgroundColor: '#1a120b', border: `2px solid ${THEME.border}`, borderRadius: 0 } } }}
            >
              <MenuItem value={10} sx={{ fontFamily: "'Press Start 2P'", fontSize: '10px', color: THEME.textMain }}>TOP 10</MenuItem>
              <MenuItem value={20} sx={{ fontFamily: "'Press Start 2P'", fontSize: '10px', color: THEME.textMain }}>TOP 20</MenuItem>
              <MenuItem value={50} sx={{ fontFamily: "'Press Start 2P'", fontSize: '10px', color: THEME.textMain }}>TOP 50</MenuItem>
              <MenuItem value={100} sx={{ fontFamily: "'Press Start 2P'", fontSize: '10px', color: THEME.textMain }}>TOP 100</MenuItem>
            </Select>
          </Stack>
        </Stack>
        ...
        >
          ...
          {!loading && processedLogs.length > 0 && (
            <Box sx={{ display: 'flex', px: 2, pb: 1, borderBottom: `2px solid ${THEME.border}`, mb: 1 }}>
              <Typography sx={{ fontFamily: "'Press Start 2P'", fontSize: 8, color: '#888', width: { xs: '60px', md: '75px' }, flexShrink: 0 }}>RANK</Typography>
              <Typography sx={{ fontFamily: "'Press Start 2P'", fontSize: 8, color: '#888', width: '120px', flexShrink: 0 }}>WORD</Typography>
              <Typography sx={{ fontFamily: "'Press Start 2P'", fontSize: 8, color: '#888', flex: 1, mx: 2 }}>USAGE METER</Typography>
              <Typography sx={{ fontFamily: "'Press Start 2P'", fontSize: 8, color: '#888', width: '60px', flexShrink: 0, textAlign: 'right' }}>COUNT</Typography>
            </Box>
          )}
