...
export const EnemyEntity = memo(
  ({
    ...
    const STORAGE_BASE_URL = "https://qsopjsioqmqtyaocqmmx.supabase.co/storage/v1/object/public/asset/img_monster/";
            ...
            {(isTargeted || selectionCount > 0) && (
              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", zIndex: 200 }}>
                <div style={{ width: "60px", height: "60px", border: "4px solid red", borderRadius: "50%", display: "flex", justifyContent: "center", alignItems: "center", background: "rgba(255, 0, 0, 0.2)", boxShadow: "0 0 15px red" }}>
                  {selectionCount > 0 && (
                    <span style={{ color: "white", fontWeight: "bold", fontSize: "24px", textShadow: "2px 2px 0 #000" }}>
              ...
              <AnimatePresence>
                {enemy.hp > 0 && gameState === "PLAYERTURN" && (
                  <motion.div
                    ...
                    style={{ position: "absolute", top: "-45px", left: "50%", zIndex: 110, pointerEvents: "auto", cursor: "help", display: "flex", flexDirection: "column", alignItems: "center" }}
                  >
                    ...
                    <AnimatePresence>
                      {showIntentTooltip && (
                        <motion.div initial={{ opacity: 0, y: 5, x: "-50%" }} animate={{ opacity: 1, y: 0, x: "-50%" }} exit={{ opacity: 0, y: 5, x: "-50%" }} style={{ position: "absolute", bottom: "120%", left: "50%", padding: "4px 10px", background: "rgba(0, 0, 0, 0.95)", color: "#fff", fontSize: "11px", fontWeight: "bold", borderRadius: "4px", whiteSpace: "nowrap", border: `1px solid ${enemy.nextAction === "guard" ? "#3498db" : "#e74c3c"}`, zIndex: 1001, boxShadow: "0 4px 15px rgba(0,0,0,0.5)", pointerEvents: "none" }}>
                          {enemy.nextAction === "guard" ? "Planning to GUARD" : "Preparing to STRIKE"}
                          <div style={{ position: "absolute", top: "100%", left: "50%", transform: "translateX(-50%)", width: "0", height: "0", borderLeft: "5px solid transparent", borderRight: "5px solid transparent", borderTop: `5px solid ${enemy.nextAction === "guard" ? "#3498db" : "#e74c3c"}` }} />
                        </motion.div>
                      )}
                    </AnimatePresence>
                  </motion.div>
