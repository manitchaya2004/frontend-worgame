      ...
      <div style={styles.flexGroup}>
        <AnimatePresence initial={false} mode="popLayout">
          {activeSelectedItems.map((item) => {
            ...
            // ✅ คำนวณ Damage จากค่าคงที่ส่วนกลาง
            const displayDamage = getLetterDamage(item.char);

            return (
              <motion.div
                key={item.id}
                layout // ✅ ช่วยให้ตัวอักษรที่เหลือขยับมาชิดกันลื่นๆ เวลาตัวข้างหน้าถูกลบ
                ...
                whileTap={{ scale: 0.9 }}
                onClick={() => store.deselectLetter(item)} // กดเพื่อเอาออก
                ...
                >
                  ...
                  <div
                    style={{
                      ...
                      color: isBlind ? "#dcdde1" : isBleed ? "#fadbd8" : isPoisoned ? "#1b5e20" : isStunned ? "#2c3e50" : "#3e2723",
                    }}
                  >
                    ...
                    <div style={styles.statusIconPos}>
                      ...
                      {isBlind && <div style={{ color: "#bdc3c7", fontSize: "11px" }}><FaEyeSlash /></div>}
                      {isBleed && (
                    ...
                    <div style={styles.damageTextPos(isPoisoned, isBlind, isBleed)}>
                      {isBlind ? "?" : displayDamage.toFixed(1)}
                    </div>
                  </div>
                </div>
              </motion.div>
...
const styles = {
  ...
  damageTextPos: (isPoisoned, isBlind, isBleed) => ({
    ...
    fontSize: "9px",
    fontWeight: "900",
    color: isBlind ? "#bdc3c7" : isBleed ? "#fadbd8" : isPoisoned ? "#2e7d32" : "#5d4037",
    zIndex: 2,
  }),
};