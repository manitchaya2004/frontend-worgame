...
export const PlayerStatusCard = ({ onHeal, onCure, onReroll }) => {
  ...
  const {
    ...
    max_mana = 0,
    shield = 0,
    potions = { health: 0, reroll: 0, cure: 0 },
    power = 0,
    speed = 0,
    deck_list = [],
    draw_pile = [],
  ...
  return (
    <div
      style={{
        ...
        gap: "8px",
        boxShadow:
          "0 4px 20px rgba(0,0,0,0.6), inset 0 0 15px rgba(61,46,36,0.1)",
        height: "fit-content",
    ...
    >
      ...
      >
        ...
        <div
          ...
          onMouseLeave={
            allowHover ? () => setIsShieldHovered(false) : undefined
          }
        ...
        <AnimatePresence>
          {allowHover && isDeckHovered && (
            ...
            >
              ...
              >
                Bright icons are remaining in draw pile.
                <br />
                Dim icons are currently on the board or used.
              </span>
            </motion.div>
          )}
        </AnimatePresence>
...
const SingleSlot = ({ index, maxSlot }) => {
  ...
  const isVowel = item
    ? ["A", "E", "I", "O", "U"].includes(item.char.toUpperCase())
    : false;
  ...
  const getBuffBadgeData = () => {
    ...
    switch (buffType) {
      ...
      case "mana-plus":
        return { icon: <GiWaterDrop />, bgColor: "#00bcd4" };
      case "shield-plus":
        return { icon: <GiTrident />, bgColor: "#e67e22" };
      case "add_bleed":
        return { icon: <GiBowieKnife />, bgColor: "#8b0000" };
      case "add_poison":
      case "add_posion":
        return { icon: <FaCloud />, bgColor: "#27ae60" };
      case "add_stun":
        return { icon: <FaBolt />, bgColor: "#f39c12" };
      case "add_blind":
        return { icon: <FaEyeSlash />, bgColor: "#8e44ad" };
      case "heal":
        return { icon: <FaPlus />, bgColor: "#2ecc71" };
      case "bless":
        return { icon: <FaCross />, bgColor: "#f1c40f" };
      case "vampire_fang":
        return { icon: <GiFangs />, bgColor: "#8b0000" };
      default:
        return null;
    }
  };
  ...
  const getTooltipBuffInfo = () => {
    switch (buffType) {
      ...
      case "add_bleed":
        return {
          desc: " 100% Chance to Bleed target (stack based)",
          color: "#e74c3c",
          icon: <GiBowieKnife />,
        };
      case "add_poison":
      case "add_posion":
        return {
          desc: " 100% Chance to Poison target (stack based)",
          color: "#2ecc71",
          icon: <FaCloud />,
        };
      case "add_stun":
        return {
          desc: " 100% Chance to Stun target (stack based)",
          color: "#f1c40f",
          icon: <FaBolt />,
        };
      case "add_blind":
        return {
          desc: " 100% Chance to Blind target (stack based)",
          color: "#8e44ad",
          icon: <FaEyeSlash />,
        };
      case "heal":
        return {
          desc: " Heal 1 HP on guard",
          color: "#2ecc71",
          icon: <FaPlus />,
        };
      case "bless":
        return {
          desc: " Cleanses 1 Debuff on guard",
          color: "#f1c40f",
          icon: <FaCross />,
        };
      case "vampire_fang":
        return {
          desc: " 100% Chance to Lifesteal 50% dmg",
          color: "#8b0000",
          icon: <GiFangs />,
        };
      default:
  ...
  const getTooltipDebuffInfo = () => {
    if (isStunned)
      return {
        desc: ` Stunned for ${duration} turn(s)`,
        color: "#f39c12",
        icon: <FaBolt />,
      };
    if (isPoisoned)
      return {
        desc: ` Takes damage (${duration} turns left)`,
        color: "#2ecc71",
        icon: <FaSkullCrossbones />,
      };
    if (isBlind)
      return {
        desc: ` Blinded (${duration} turns left)`,
        color: "#8e44ad",
        icon: <FaEyeSlash />,
      };
    if (isBleed)
      return {
        desc: ` Bleeding (${duration} turns left)`,
        color: "#e74c3c",
        icon: <FaTint />,
      };
    return null;
          ...
          {item && !isLocked && (
            <motion.div
              ...
              animate={{
                ...
                scale: 1,
                filter: isStunned
                  ? "brightness(1.1) drop-shadow(0 0 4px rgba(243, 156, 18, 0.4))"
                  : "none",
              }}
              exit={{ opacity: 0, scale: 0.2 }}
              whileHover={
                !isDisabled && !isStunned && allowHover ? { scale: 1.05 } : {}
              }
              onClick={onSelect}
              style={{
                width: "92%",
              ...
              <span
                style={{
                  ...
                  textShadow: isBlind
                    ...
                    : "0.5px 1px 0px rgba(255,255,255,0.5)",
                  lineHeight: 1,
              ...
              <AnimatePresence>
                {(isPoisoned || isStunned || isBlind || isBleed) && (
                  ...
                  >
                    ...
                    {isStunned && (
                      <FaBolt style={{ color: "#fff", fontSize: "11px" }} />
                    )}
                    {isBlind && (
                      <FaEyeSlash
                    ...
                    {isBleed && (
                      <FaTint style={{ color: "#922b21", fontSize: "11px" }} />
                    )}
                  </motion.div>
                )}
              </AnimatePresence>
          ...
          >
            ...
            >
              {tooltipBuff && (
                ...
                >
                  <div style={{ color: tooltipBuff.color, fontSize: "12px" }}>
                    {tooltipBuff.icon}
                  </div>
                  <span
                    style={{
              ...
              {tooltipDebuff && (
                <div
                  style={{ display: "flex", alignItems: "center", gap: "6px" }}
                >
                  <div style={{ color: tooltipDebuff.color, fontSize: "12px" }}>
                    {tooltipDebuff.icon}
                  </div>
                  <span
                    style={{
...
export const InventorySlot = () => {
  ...
  // 🌟 ล็อกอยู่ที่ 16 ช่อง (4x4)
  const calculatedMaxSlot = 16;
  ...
  return (
    ...
    >
      ...
      >
        <GiBrain
          style={{
            color: isEnemyTurn ? "#e74c3c" : "#d4af37",
            fontSize: "18px",
          }}
        />
        <div
